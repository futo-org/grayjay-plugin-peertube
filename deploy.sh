...
# Fetch instances and extract hosts into a temporary array
 curl -s "https://instances.joinpeertube.org/api/v1/instances?start=0&count=1000" |   jq -r '.data[].host' |   awk 'BEGIN {print "// This content is autogenerated during deployment"; print "INDEX_INSTANCES.instances = ["}
       {print "        \"" $0 "\","}
